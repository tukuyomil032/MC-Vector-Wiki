...
function classifyAssets(assets: ReleaseAsset[]): Platform[] {
  ...
  const tarGz = assets.filter(
    (a) => a.name.endsWith('.tar.gz') || a.name.endsWith('.app.tar.gz')
  )
  const winExe = assets.filter(
